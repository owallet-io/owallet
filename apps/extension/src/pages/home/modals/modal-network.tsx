...
import { COINTYPE_NETWORK, ChainIdEnum, getKeyDerivationFromAddressType, unknownToken } from '@owallet/common';
...
export const ModalNetwork: FC<{
  ...
  const { chainStore, accountStore, hugeQueriesStore, priceStore, keyRingStore } = useStore();
  ...
  const chainsInfoWithBalance = chainStore.chainInfos.map((item, index) => {
    let balances = hugeQueriesStore.allKnownBalances.filter(token => token.chainInfo.chainId === item.chainId);
      ...
      if (getDevicesHID.length) {
        if (
          ...
          await keyRingStore.setKeyStoreLedgerAddress(
            `${keyDerivation}'/${chainInfo.bip44.coinType ?? chainInfo.coinType}'/${selected.bip44HDPath.account}'/${
  ...
  const switchChain = async chainInfo => {
    try {
      ...
      if (chainInfo.chainId !== chainStore.current.chainId && chainInfo.chainId !== 'isAll') {
    ...
    >
      <div className={styles.contentWrap}>
          ...
          {typeNetwork.map((item, index) => (
            <div
              ...
              className={classnames([
                ...
                tab.id === item.id ? styles.activeBorderBottom : styles.inactiveBorderBottom
              ])}
            >
              <span
                className={classnames([
                  styles.titleTxtItem,
                  tab.id === item.id ? styles.activeTxtColor : styles.inactiveTxtColor
                ])}
              >
                {item.title}
              </span>
        ...
        <div className={styles.containerListChain}>
          {chains?.length > 0 &&
            [...allNetworkData, ...sortChainsByPrice(chains)].map((item, index) => {
              return (
                <div
                  ...
                  className={classnames([
                    styles.itemChain,
                    item.chainId === (chainStore.isAllNetwork ? 'isAll' : chainStore.current.chainId)
                ...
                >
                  <div className={styles.leftBlockHuge}>
                    <div className={styles.wrapImgChain}>
                      <img className={styles.imgChain} src={item?.chainSymbolImageUrl || unknownToken.coinImageUrl} />
                    </div>
                  ...
                  <div className={styles.rightBlockHuge}>
                    <input
                      id={item.chainId}
                      defaultChecked={item.chainId === (chainStore.isAllNetwork ? 'isAll' : chainStore.current.chainId)}
                      name={'chain'}
