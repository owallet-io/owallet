...
export const SignPage: FunctionComponent = observer(() => {
  ...
  const { chainStore, keyRingStore, signInteractionStore, accountStore, queriesStore, priceStore } = useStore();
  ...
  useEffect(() => {
    if (signInteractionStore.waitingData) {
      ...
      if (data.data.signDocWrapper.payer && data.data.signDocWrapper.payer !== data.data.signer) {
        feeConfig.setDisableBalanceCheck(true);
      }
      // We can't check the fee balance if the granter is not the signer.
      if (data.data.signDocWrapper.granter && data.data.signDocWrapper.granter !== data.data.signer) {
        feeConfig.setDisableBalanceCheck(true);
      }
  ...
  return (
    ...
    >
      ...
      <div className={cx('setting', dataSetting ? 'activeSetting' : '', 'modal')} ref={dataRef}>
        <DataModal
          onClose={() => {
            handleCloseDataModal();
          }}
        ...
        isLoaded ? (
          <div className={style.container}>
            <div style={{ height: '75%', overflow: 'scroll', padding: 16 }}>
              <div className={classnames(style.tabs)} style={{ display: 'flex', paddingBottom: 12 }}>
                <div>
                      ...
                      <div style={{ flexDirection: 'column', display: 'flex' }}>
                        ...
                        <Text color={colors['neutral-text-body']}>
                          <Address maxCharacters={18} lineBreakBeforePrefix={false}>
                            {accountInfo.walletStatus === WalletStatus.Loaded && addressDisplay
                              ? addressDisplay
                              : '...'}
                          </Address>
                        </Text>
