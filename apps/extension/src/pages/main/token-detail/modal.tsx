...
export const TokenDetailModal: FunctionComponent<{
    ...
    const balance = (() => {
        if ("cosmos" in modularChainInfo) {
            ...
            const isBtcLegacy = denomHelper.type === "legacy";
            if (isBtcLegacy) {
                return queryBalances.getQueryBtcLegacyAddress(
                    account.btcLegacyAddress
                ).getBalance(currency);
            } else if (chainInfo.features.includes("tron")) {
                return queryBalances.getQueryEthereumHexAddress(
                    account.ethereumHexAddress
                ).getBalance(currency);
            } else {
                return queryBalances.getQueryByAddress(
    ...
    return (
        <Styles.Container>
          ...
          {(() => {
            ...
            >
                <ReceiveModal coinMinimalDenom={coinMinimalDenom} chainId={chainId}
                              close={() => setIsReceiveOpen(false)}/>
            </Modal>
        </Styles.Container>
    );
