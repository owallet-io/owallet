import { IChainInfoImpl, ObservableQuery, QuerySharedContext } from '@owallet/stores';
...
export class ObservableQueryChains extends ObservableQuery<ChainsResponse> {
  ...
  isPFMEnabled = computedFn((chainId: string): boolean => {
    const chain = this.chains.find(chain => {
      return chain.chainInfo.chainIdentifier === ChainIdHelper.parse(chainId).identifier;
    });
    if (!chain) {
      return false;
    }
