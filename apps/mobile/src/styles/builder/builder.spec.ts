...
describe('Test style builder', () => {
  ...
  test('Test Conditional Flatten', () => {
    ...
    expect(builder.flatten(['test', 'absolute', 'test2'], [null, undefined, false, true])).toStrictEqual({
      position: 'absolute',
      fontSize: 22
    });

    expect(builder.flatten(['test', 'absolute'], ['test2', undefined, false, true])).toStrictEqual({
      position: 'absolute',
      fontSize: 22
    });

    expect(builder.flatten(['test', 'absolute'], [null, undefined, 'test2', true])).toStrictEqual({
      position: 'absolute',
      fontSize: 22
    });

    expect(builder.flatten(['absolute'], [null, undefined, 'test2', true])).toStrictEqual({
      position: 'absolute',
      fontSize: 22
    });

    expect(builder.flatten(['absolute'], ['test', undefined, 'test2', true])).toStrictEqual({
      position: 'absolute',
      fontSize: 22
    });
  });
  ...
  test('Test Text static style', () => {
    ...
    const weightFonts = [
      'SpaceGrotesk-Thin',
      'SpaceGrotesk-SemiLight',
      'SpaceGrotesk-Light',
      'SpaceGrotesk-Regular',
      'SpaceGrotesk-Medium',
      'SpaceGrotesk-SemiBold',
      'SpaceGrotesk-Bold',
      'SpaceGrotesk-ExtraBold',
      'SpaceGrotesk-Black'
    ];
