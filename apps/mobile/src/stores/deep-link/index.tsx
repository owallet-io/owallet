...
export class DeepLinkStore {
  ...
  protected async processDeepLinkURL(_url: string) {
    try {
      ...
      if (url.protocol === "owallet:" && url.host === "wcV2") {
        await this.walletConnectStore.processDeepLinkURL(url);
      }

      // url.host === 'staking' is from Deeplink owallet://staking?
      // url.host === 'deeplink.owallet.io' is from applink or universal link https://deeplink.owallet.io/staking
      if (
        (url.protocol === "owallet:" && url.host === "staking") ||
        (url.host === "deeplink.owallet.io" && url.pathname === "/staking")
      ) {
        this.processStakingLinkURL(url);
      }

      if (
        (url.protocol === "owallet:" && url.host === "web-browser") ||
        (url.host === "deeplink.owallet.io" && url.pathname === "/web-browser")
      ) {
        this.processWebBrowserLinkURL(url);
      }

      if (
        (url.protocol === "owallet:" && url.host === "show-address") ||
        (url.host === "deeplink.owallet.io" && url.pathname === "/show-address")
      ) {
