...
export class WCMessageRequester implements MessageRequester {
  ...
  static getVirtualURL = (id: string): string => {
    return `https://owallet_wc_virtual@2.${id}`;
  };

  static isVirtualURL = (url: string): boolean => {
    return url.startsWith("https://owallet_wc_virtual@2.");
  };
  ...
  static getIdFromVirtualURL = (url: string): string => {
    ...
    return url.replace("https://owallet_wc_virtual@2.", "").replace("/", "");
  };
  ...
  async sendMessage<M extends Message<unknown>>(
    ...
    if (result.error) {
      if (typeof result.error === "string") {
        ...
        // else {
        //   throw new EthereumProviderRpcError(result.error.code, result.error.message, result.error.data);
        // }
      }
    }
