...
import { _keyExtract, getTokenInfos, maskedNumber } from '../../../utils/helper';
...
import { chainIcons, ChainIdEnum, getTotalUsd } from '@oraichain/oraidex-common';
...
export const SelectNetworkModal: FunctionComponent<{
  ...
  useEffect(() => {
    let tmpChainInfos = [];
    chainStore.chainInfosInUI.map(c => {
      if (!c.chainName.toLowerCase().includes('test') && c.chainName.toLowerCase().includes(keyword.toLowerCase())) {
        tmpChainInfos.push(c);
      }
    });
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            {chainIcon ? (
              <OWIcon type="images" style={{ borderRadius: 999 }} source={{ uri: chainIcon.Icon }} size={28} />
            ) : (
  ...
  useEffect(() => {
    if (chainAssets) {
      const sortedData = Object.entries(chainAssets).sort((a, b) => b[1].sum - a[1].sum);
