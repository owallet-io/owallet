...
import { TokenItemType, toDisplay, tokensIcon } from '@oraichain/oraidex-common';
...
}> = registerModal(({ close, activeToken, data, setToken, selectedChainFilter }) => {
  const safeAreaInsets = useSafeAreaInsets();
  const { universalSwapStore, appInitStore, accountStore, keyRingStore, chainStore } = useStore();
    ...
    if (key && chain && key !== '' && chain !== '') {
      ...
      data.map(d => {
        if (
          ...
          (d.chainId + d.denom + d.name + d.org + d.coinGeckoId).toString().toLowerCase().includes(key.toLowerCase())
        ) {
          tmpData.push(d);
        }
      });
      ...
      if (chain && chain !== '') {
        const tmpData = data.filter(d => d.chainId.toString().toLowerCase().includes(chain.toLowerCase()));
        handleSetTokensWithAmount(tmpData);
        return;
      }
      setTokens(data);
  ...
  const handleSetTokensWithAmount = useCallback(
    tokens => {
      ...
      tokens.map(t => {
        ...
        const amount = toDisplay(universalSwapStore?.getAmount?.[t.denom], t.decimals);

        const totalUsd = usdPrice ? (Number(amount) * Number(usdPrice)).toFixed(2) : 0;
  ...
  const renderTokenItem = useCallback(
    item => {
      if (item) {
        ...
        const tokenIconFromLocal = currencies.find(tk => tk.coinGeckoId === item.coinGeckoId);
        return (
          <>
            <TouchableOpacity
              onPress={() => {
              ...
              style={[
                styles.btnItem,
                activeToken.coinGeckoId === item.coinGeckoId ? styles.active : { paddingHorizontal: 16 }
              ]}
            >
              <View style={styles.leftBoxItem}>
                ...
                >
                  <OWIcon
                    type="images"
                    source={{
                      uri: tokenIcon?.Icon || tokenIconFromLocal?.coinImageUrl || unknownToken.coinImageUrl
                    }}
                    size={30}
                    style={{
                      borderRadius: 999
            ...
            {activeToken.coinGeckoId === item.coinGeckoId ? null : <View style={styles.borderLine} />}
          </>
        );
      }
    },
    [universalSwapStore?.getAmount, appInitStore.getInitApp.theme, appInitStore.getInitApp.wallet]
  );
