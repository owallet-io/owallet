...
import { renderDApps, renderScene } from '@src/screens/web/routes';
  ...
  const renderTabBar = props => (
    <TabBar
      ...
      renderLabel={({ route, focused, color }) => (
        <OWText style={{ color, fontSize: 16, fontWeight: '600' }}>{route.title}</OWText>
      )}
      // scrollEnabled={true}
      activeColor={colors['primary-surface-default']}
      inactiveColor={colors['neutral-text-title']}
    />
  );
