  ...
  const renderItemSkeleton = () => {
    return (
      <SkeletonPlaceholder
        ...
        borderRadius={12}
      >
        <SkeletonPlaceholder.Item width={'100%'} marginVertical={8} height={65}></SkeletonPlaceholder.Item>
      </SkeletonPlaceholder>
    );
  };
  ...
  return !item ? null : (
    ...
    >
      {!inject ? (
        ...
        <TouchableOpacity disabled={!inject} onPress={() => onToBrowser(item.url)}>
          <ImageBackground
            style={{
              width: '100%',
              height: 160
          ...
          >
            ...
            >
              {item.logo ? <OWIcon type={'images'} source={item.logo} size={32} /> : null}
              <OWText
                size={16}
                weight={'600'}
                style={{
