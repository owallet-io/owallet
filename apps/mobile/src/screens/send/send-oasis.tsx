...
export const SendOasisScreen: FunctionComponent<{
  ...
  const { chainStore, oasisAccountStore, queriesStore, appInitStore } = useStore();
  ...
  useEffect(() => {
    ...
    navigation.setOptions({
      headerTitle: () => <OWHeaderTitle title={'Send'} subTitle={chainInfo.chainName} />
    });
  }, [chainId]);
  ...
  const balance = queryBalances.getQueryBech32Address(sender).getBalance(currency);

  console.log('balance', balance);

  const intl = useIntl();
  const sendConfigs = useSendOasisTxConfig(chainStore, queriesStore, chainId, sender, 1);
