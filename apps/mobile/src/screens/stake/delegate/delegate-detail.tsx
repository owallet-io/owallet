...
export const DelegateDetailScreen: FunctionComponent<DelegateDetailProps> = observer(({}) => {
  ...
  const unbondingValidators = queries.cosmos.queryValidators.getQueryStatus(Staking.BondStatus.Unbonding);
  const unbondedValidators = queries.cosmos.queryValidators.getQueryStatus(Staking.BondStatus.Unbonded);
  const thumbnail =
  ...
  useEffect(() => {
    navigation.setOptions({
      headerTitle: () => <OWHeaderTitle title={'delegate'} subTitle={chainStore.current?.chainName} />
    });
