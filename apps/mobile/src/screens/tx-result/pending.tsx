...
import { View, Image, ScrollView, InteractionManager, StyleSheet } from 'react-native';
...
export const TxPendingResultScreen: FunctionComponent = observer(() => {
  const { chainStore, txsStore, accountStore, keyRingStore, priceStore, queriesStore } = useStore();
  ...
  const { params } = route;
  const accountAddress = accountStore.getAccount(chainId).getAddressDisplay(keyRingStore.keyRingLedgerAddresses, false);
  ...
  const amount = new CoinPretty(params?.data?.currency, new Dec(params?.data?.amount?.amount));
  const chainTxs =
    ...
    if (chainInfo?.raw?.txExplorer?.txUrl === '') {
      if (params?.data?.type === 'send') {
        const bal = queries.queryBalances
          ...
          .balances.find(bal => bal.currency.coinMinimalDenom === data?.currency?.coinMinimalDenom);
        if (bal) {
          bal.fetch();
        }
      }
  ...
  useEffect(() => {
    if (txHash && chainInfo?.raw?.txExplorer?.txUrl !== '') {
          ...
          setTimeout(() => {
            if (params?.data?.type === 'send') {
              ...
              Promise.all([
                ...
                queries.cosmos.queryDelegations.getQueryBech32Address(accountAddress).fetch(),
                queries.cosmos.queryRewards.getQueryBech32Address(accountAddress).fetch(),
                queries.cosmos.queryUnbondingDelegations.getQueryBech32Address(accountAddress).fetch()
              ]);
            }
            navigate(SCREENS.TxSuccessResult, {
              chainId,
  ...
  const fee = () => {
    if (params?.data?.fee) {
      return new CoinPretty(chainInfo.feeCurrencies?.[0], new Dec(params?.data?.fee.amount?.[0]?.amount));
    } else {
      if (data?.stdFee?.amount?.[0]?.amount) {
        return new CoinPretty(chainInfo.feeCurrencies?.[0], new Dec(data?.stdFee?.amount?.[0]?.amount));
      }
  ...
  const dataItem =
    ...
    _.pickBy(params?.data, function (value, key) {
      return key !== 'memo' && key !== 'fee' && key !== 'amount' && key !== 'currency' && key !== 'type';
    });
      ...
      <View style={styles.containerBox}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <HeaderTx
            ...
            amount={`${params?.data?.type === 'send' ? '-' : ''}${amount?.shrink(true)?.trim(true)?.toString()}`}
          ...
          <View style={styles.cardBody}>
              ...
              Object.keys(dataItem).map(function (key) {
                return (
                  <ItemReceivedToken
                    ...
                    valueDisplay={dataItem?.[key] && formatContractAddress(dataItem?.[key], 20)}
                    value={dataItem?.[key]}
                  />
                );
              })}
            ...
            <ItemReceivedToken
              ...
              btnCopy={false}
            />
            <ItemReceivedToken label={'Memo'} valueDisplay={params?.data?.memo || '-'} btnCopy={false} />
          </View>
        </ScrollView>
      </View>
    </PageWithBottom>
