...
export const amountToAmbiguousString = (amount: { toDec: () => Dec }) => {
  ...
  if (amount.toDec().lt(new Dec(10))) {
    ...
    return `${n.mul(new Int(10)).toString()}~${n.add(new Int(1)).mul(new Int(10)).toString()}`;
  } else {
    const n = amount.toDec().quo(new Dec(100)).truncate();
    return `${n.mul(new Int(100)).toString()}~${n.add(new Int(1)).mul(new Int(100)).toString()}`;
  }
};
...
export const amountToAmbiguousAverage = (amount: { toDec: () => Dec }) => {
  ...
  const i = str.indexOf('~');
  if (i >= 0) {
    return (parseInt(str.substring(0, i), 10) + parseInt(str.substring(i + 1), 10)) / 2;
  }
