  ...
  observer(({ close, feeConfig, gasConfig }) => {
    ...
    return (
      <WrapViewModal title="Set Fee" disabledScrollView={false}>
        ...
        <FeeButtons label="Fee" gasLabel="Gas" feeConfig={feeConfig} vertical gasConfig={gasConfig} />
...
export const FeeInSign: FunctionComponent<{
  ...
  console.log('feeConfig.fee', feeConfig?.fee?.toString());
    ...
    <React.Fragment>
      ...
      <ItemDetail
        ...
        value={
          ...
          >
            <Text weight={'600'} color={colors['primary-text-action']} size={16}>
              {feeConfig.feeType ? `${capitalizedText(feeConfig.feeType)} :` : ''}{' '}
              {feePrice ? feePrice.toString() : '-'}
            </Text>
            {canFeeEditable ? (
              <View style={style.flatten(['margin-left-6'])}>
                <DownArrowIcon color={colors['primary-text-action']} height={20} />
              </View>
            ) : null}
          </TouchableOpacity>
        }
