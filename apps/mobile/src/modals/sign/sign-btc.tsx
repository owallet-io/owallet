...
import { Utxos, UtxosWithNonWitness, UtxosWithWitness } from '@owallet/stores-btc/build/queries/types';
...
export const SignBtcModal = registerModal(
  observer<{
    ...
    const { chainStore, uiConfigStore, bitcoinAccountStore, signBtcInteractionStore, queriesStore, appInitStore } =
      useStore();
    ...
    const utxos = queriesStore.get(chainId).bitcoin.queryBtcUtxos.getQueryBtcAddress(signer).utxos;
    ...
    const getInputsAndOutput = () => {
      ...
      const { inputs, outputs } = accumulative(utxosData, targetOutputs, Number(feeRate.roundUp().toString()));
      setInputs(inputs);
      setOutputs(outputs);
      return { inputs, outputs };
    };
    const fetchUxtosNonWitness = async (utxos: Utxos[]): Promise<UtxosWithNonWitness[]> => {
      return Promise.all(
        utxos.map(async item => {
          try {
            // Fetch the nonWitnessUtxo data (assuming the response contains the raw transaction hex as a string)
            const res = await simpleFetch<string>(chainInfo.rest, `/tx/${item.txid}/hex`);
            return {
              ...item,
    ...
    const fetchUxtosWithHex = async (utxos: Utxos[]): Promise<UtxosWithNonWitness[]> => {
      return Promise.all(
        utxos.map(async item => {
          try {
            // Fetch the nonWitnessUtxo data (assuming the response contains the raw transaction hex as a string)
    ...
    useEffect(() => {
      ...
      const unsignedTx: UnsignedBtcTransaction = JSON.parse(Buffer.from(data.message).toString());
      if (unsignedTx) {
      ...
      >
        <Box style={style.flatten(['padding-12', 'padding-top-0'])}>
          ...
          {(() => {
            ...
            return (
              <FeeControl feeConfig={feeConfig} senderConfig={senderConfig} gasConfig={null} gasSimulator={null} />
            );
          })()}
          <Gutter size={12} />
          ...
          <XAxis>
            <OWButton
              ...
              onPress={async () => {
                await signBtcInteractionStore.rejectWithProceedNext(interactionData.id, () => {});
              }}
            />
            ...
            <OWButton
              ...
              style={{ flex: 1, width: '100%' }}
              onPress={approve}
              loading={signBtcInteractionStore.isObsoleteInteraction(interactionData.id) || isLedgerInteracting}
            />
          </XAxis>
          {/*<Gutter size={24} />*/}
        </Box>
