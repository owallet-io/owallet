...
export const InteractionModalsProivder: FunctionComponent = observer(({ children }) => {
  const { ledgerInitStore, permissionStore, signInteractionStore, modalStore } = useStore();

  console.log('signInteractionStore', signInteractionStore);
  ...
  const renderAccessModal = () => {
    // if (permissionStore.waitingDatas && navigationRef?.current?.getCurrentRoute().name === 'Web.dApp') {
    if (permissionStore.waitingDatas) {
      return permissionStore.waitingDatas.map(wd => {
        return <AccessModal waitingData={wd} isOpen={true} close={() => permissionStore.rejectAll()} />;
      });
    }
  };
  ...
  return (
    <React.Fragment>
      {ledgerInitStore.isInitNeeded ? (
        <LedgerGranterModal isOpen={true} close={() => ledgerInitStore.abortAll()} />
      ) : null}

      {renderAccessModal()}

      {signInteractionStore.waitingData ? (
        <SignModal isOpen={true} close={() => signInteractionStore.rejectAll()} />
      ) : null}
      ...
      {signInteractionStore.waitingTronData ? (
        <SignTronModal isOpen={true} close={() => signInteractionStore.rejectAll()} />
      ) : null}

      {signInteractionStore.waitingBitcoinData ? (
        <SignBitcoinModal isOpen={true} close={() => signInteractionStore.rejectAll()} />
      ) : null}

      {modalStore.getOptions?.isOpen ? (
        <HomeBaseModal {...modalStore.getOptions} isOpen={true} close={() => modalStore.close()} />
      ) : null}
