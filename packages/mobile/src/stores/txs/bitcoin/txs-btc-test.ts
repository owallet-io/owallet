  ...
  async getTxs(page: number, current_page: number, params: ParamsFilterReqTxs): Promise<Partial<ResTxs>> {
    try {
      ...
      if (data?.txs?.length > 0) {
        ...
        console.log('🚀 ~ file: txs-btc-test.ts:32 ~ TxsBtcTestNet ~ params?.addressAccount:', params?.addressAccount);
        console.log('🚀 ~ file: txs-btc-test.ts:28 ~ TxsBtcTestNet ~ data?.txs:', data?.txs);
        console.log('🚀 ~ file: txs-btc-test.ts:30 ~ TxsBtcTestNet ~ this.currentChain:', this.currentChain);
