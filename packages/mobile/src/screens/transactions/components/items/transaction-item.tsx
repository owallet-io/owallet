...
import { formatContractAddress, getDataFromDataEvent, getValueFromDataEvents, limitString } from '@src/utils/helper';
...
const OWTransactionItem = observer(({ item, time, ...props }: IOWTransactionItem) => {
      ...
      <View style={styles.item}>
        <View style={[styles.flexRow, { paddingBottom: 5 }]}>
          ...
          {!!itemTransfer?.typeEvent ? (
            <Text variant="body2" typo="regular" color={colors['title-modal-login-failed']}>
              <Text color={colors['green-500']}>{item?.countTypeEvent > 0 ? `+${item?.countTypeEvent}` : null}</Text>{' '}
            ...
            >
              ...
              <View style={styles.iconstyle}>
                <OWIcon
                  size={12}
                  color={item?.status === 'success' ? colors['green-500'] : colors['orange-800']}
                  name={item?.status === 'success' ? 'check_stroke' : 'close_shape'}
                />
              </View>
            </View>
          )}
        ...
        <View style={styles.flexRow}>
          <Text
            ...
            style={itemTransfer?.amount && (itemTransfer?.isPlus || itemTransfer?.isMinus) && styles.amount}
          >
            {`${
              itemTransfer?.amount && itemTransfer?.isPlus && !itemTransfer?.isMinus
                ? '+'
