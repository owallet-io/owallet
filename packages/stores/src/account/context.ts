import { OWallet, Key, SettledResponse, SettledResponses } from '@owallet/types';
import { DebounceActionTimer } from '@owallet/mobx-utils';

export class AccountSharedContext {
  protected suggestChainDebounceTimer = new DebounceActionTimer<[fn: () => Promise<void>], void>(0, async requests => {
  ...
  protected enableDebounceTimer = new DebounceActionTimer<[chainId: string], void>(0, async requests => {
  ...
  protected getKeyDebounceTimer = new DebounceActionTimer<[chainId: string], Key>(0, async requests => {
    const owallet = await this.getOWallet();
  ...
  constructor(protected readonly _getOWallet: () => Promise<OWallet | undefined>) {}
  ...
  getKey(chainId: string, action: (res: SettledResponse<Key>) => void): Promise<void> {
