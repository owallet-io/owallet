...
export const BtcDetailsTab: FunctionComponent<{ dataSign; intl }> = observer(({ dataSign, intl }) => {
  return (
    <div className={styleDetailsTab.container}>
      <Label for="signing-messages" className="form-control-label" style={{ display: 'flex' }}>
        <FormattedMessage id="sign.list.messages.label" />
        <Badge className={classnames('ml-2', styleDetailsTab.msgsBadge)}>{JSON.stringify(dataSign).length}</Badge>
      </Label>
            ...
            >
              <FormattedMessage
                ...
                values={{
                  ...
                  amount: dataSign?.data?.amount + ' ' + dataSign?.data?.currency?.coinDenom
                }}
              />
            </MsgRender>
          )}
          {!dataSign?.data?.currency && <div style={{ width: 375 }}>{JSON.stringify(dataSign, null)}</div>}
