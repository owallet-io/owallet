...
export class FeeConfig extends TxChainSetter implements IFeeConfig {
  ...
  protected get fee():
    ...
    if ('type' in this._fee) {
      ...
      const currency = this.chainGetter.getChain(this.chainId).forceFindCurrency(coinMinimalDenom);
    ...
    if (this.canOsmosisTxFeesAndReady()) {
      ...
      if (queryOsmosis) {
        ...
        return this.chainInfo.feeCurrencies
          ...
          .sort((cur1, cur2) => {
            if (cur1.coinMinimalDenom === queryOsmosis.queryTxFeesBaseDenom.baseDenom) {
              return -1;
            }
            if (cur2.coinMinimalDenom === queryOsmosis.queryTxFeesBaseDenom.baseDenom) {
              return 1;
            }
      ...
      if (queryCosmos) {
        ...
        const firstFeeDenom =
          this.chainInfo.feeCurrencies.length > 0 ? this.chainInfo.feeCurrencies[0].coinMinimalDenom : '';
  ...
  getFeePrimitive(): {
    ...
    if (!this.fee) {
      ...
      res = [
        {
          amount: this.getFeeTypePrettyForFeeCurrency(this.fee.currency, this.fee.type).toCoin().amount,
      ...
      res = this.fee.map(fee => {
        return {
          amount: fee
            .add(this.l1DataFee?.quo(DecUtils.getTenExponentN(fee.currency.coinDecimals)) ?? new Dec(0))
            .toCoin().amount,
    ...
    if (
      ...
      if (etcQueries && etcQueries.queryTerraClassicTaxRate.response && etcQueries.queryTerraClassicTaxCaps.response) {
        ...
        if (taxRate && taxRate.toDec().gt(new Dec(0))) {
          ...
          for (const sendAmt of sendAmount) {
            if (new DenomHelper(sendAmt.currency.coinMinimalDenom).type === 'native') {
              let tax = sendAmt
                .toDec()
                .mul(DecUtils.getTenExponentN(sendAmt.currency.coinDecimals))
                .mul(taxRate.toDec());
              const taxCap = etcQueries.queryTerraClassicTaxCaps.getTaxCaps(sendAmt.currency.coinMinimalDenom);
              ...
              if (taxAmount.isPositive()) {
                const i = res.findIndex(f => f.currency.coinMinimalDenom === sendAmt.currency.coinMinimalDenom);
                if (i >= 0) {
                  res[i] = {
  ...
  protected canFeeMarketTxFeesAndReady(): boolean {
    if (this.chainInfo.hasFeature('feemarket')) {
      const queries = this.queriesStore.get(this.chainId);
      if (!queries.cosmos) {
        console.log('Chain has feemarket feature. But no cosmos queries provided.');
        return false;
      }
      ...
      return queryFeeMarketGasPrices.gasPrices.find(gasPrice => this.chainInfo.findCurrency(gasPrice.denom)) != null;
    }
  ...
  protected canEIP1559TxFeesAndReady(isRefresh?: boolean): boolean {
    if (this.chainInfo.evm && this.senderConfig.sender.startsWith('0x')) {
      ...
      const feeHistoryQuery = queries.ethereum.queryEthereumFeeHistory.getQueryByFeeHistoryParams(
      ...
      const latestBaseFeePerGas = parseInt(
        feeHistoryQuery.feeHistory?.baseFeePerGas?.[feeHistoryQuery.feeHistory?.baseFeePerGas.length - 1] ?? '0'
      );
      ...
      if (feeHistoryQuery.feeHistory != null && latestBaseFeePerGas !== 0) {
        ...
        const maxPriorityFeePerGasQuery = queries.ethereum.queryEthereumMaxPriorityFee;
        ...
        const blockQuery = queries.ethereum.queryEthereumBlock.getQueryByBlockNumberOrTag(ETH_FEE_HISTORY_NEWEST_BLOCK);
        const latestBaseFeePerGas = parseInt(blockQuery.block?.baseFeePerGas ?? '0');
        const maxPriorityFeePerGasQuery = queries.ethereum.queryEthereumMaxPriorityFee;
        if (
  ...
  readonly getFeeTypePrettyForFeeCurrency = computedFn((feeCurrency: FeeCurrency, feeType: FeeType) => {
  ...
  readonly getGasPriceForFeeCurrency = computedFn((feeCurrency: FeeCurrency, feeType: FeeType): Dec => {
    if (this.chainInfo.hasFeature('osmosis-base-fee-beta') || this.canOsmosisTxFeesAndReady()) {
      const queryOsmosis = this.queriesStore.get(this.chainId).osmosis;
      if (queryOsmosis) {
        const baseDenom = queryOsmosis.queryTxFeesBaseDenom.baseDenom;
        let baseFeeCurrency =
          this.selectableFeeCurrencies.find(c => c.coinMinimalDenom === baseDenom) || this.chainInfo.feeCurrencies[0];
        ...
        if (this.chainInfo.hasFeature('osmosis-base-fee-beta')) {
          ...
          if (baseFee) {
            const low = remoteBaseFeeStep.response?.data.low
              ? parseFloat(baseFee.mul(new Dec(remoteBaseFeeStep.response.data.low)).toString(8))
              : baseFeeCurrency.gasPriceStep?.low ?? DefaultGasPriceStep.low;
            const average = Math.max(
              low,
              remoteBaseFeeStep.response?.data.average
                ? parseFloat(baseFee.mul(new Dec(remoteBaseFeeStep.response.data.average)).toString(8))
                : baseFeeCurrency.gasPriceStep?.average ?? DefaultGasPriceStep.average
            );
            const high = Math.max(
              average,
              remoteBaseFeeStep.response?.data.high
                ? parseFloat(baseFee.mul(new Dec(remoteBaseFeeStep.response.data.high)).toString(8))
        ...
        if (this.canOsmosisTxFeesAndReady()) {
          if (
            ...
            queryOsmosis.queryTxFeesFeeTokens.isTxFeeToken(feeCurrency.coinMinimalDenom)
          ) {
            const baseGasPriceStep = baseFeeCurrency.gasPriceStep ?? DefaultGasPriceStep;

            const baseGasPrice = new Dec(baseGasPriceStep[feeType].toString());
            const spotPriceDec = queryOsmosis.queryTxFeesSpotPriceByDenom.getQueryDenom(
        ...
        if (feeCurrency.coinMinimalDenom === baseFeeCurrency.coinMinimalDenom) {
      ...
      if (queryCosmos) {
        ...
        const gasPrice = gasPrices.find(gasPrice => gasPrice.denom === feeCurrency.coinMinimalDenom);
        if (gasPrice) {
          let multiplication = {
            low: 1.1,
            average: 1.2,
          ...
          if (multificationConfig.response) {
            ...
            const specific = multificationConfig.response.data[this.chainInfo.chainIdentifier];
            if (
              specific &&
              specific.low != null &&
  ...
  readonly getEIP1559TxFees = computedFn((feeTypeOrManual: FeeType | 'manual') => {
    const feeType = feeTypeOrManual === 'manual' ? 'average' : feeTypeOrManual;
    console.log('feeType', feeType);

    const ethereumQueries = this.queriesStore.get(this.chainId).ethereum;
    console.log('this.canEIP1559TxFeesAndReady()', this.canEIP1559TxFeesAndReady());

    if (ethereumQueries && this.canEIP1559TxFeesAndReady()) {
      const feeHistory = ethereumQueries.queryEthereumFeeHistory.getQueryByFeeHistoryParams(
        ETH_FEE_HISTRORY_BLOCK_COUNT,
        ETH_FEE_HISTORY_NEWEST_BLOCK,
        ETH_FEE_HISTORY_REWARD_PERCENTILES
      ).feeHistory;
      ...
      const latestBaseFeePerGas = parseInt(feeHistory?.baseFeePerGas?.[feeHistory?.baseFeePerGas.length - 1] ?? '0');
      ...
      if (feeHistory != null && latestBaseFeePerGas !== 0) {
          ...
          priorityFeeByRewardPercentileIndex =>
            priorityFeeByRewardPercentileIndex[
              ETH_FEE_HISTORY_REWARD_PERCENTILES.findIndex(
                percentile => percentile === ETH_FEE_SETTINGS_BY_FEE_TYPE[feeType].percentile
              )
            ]
        );
        ...
        const maxPriorityFeePerGasDec = new Dec(BigInt(medianOfPriorityFeesPerGas));
        ...
        if (maxPriorityFeePerGasDec.isZero()) {
          ...
          if (maxPriorityFeePerGas != null) {
            const maxPriorityFeePerGasDec = new Dec(BigInt(maxPriorityFeePerGas));
        ...
        const block = ethereumQueries.queryEthereumBlock.getQueryByBlockNumberOrTag(ETH_FEE_HISTORY_NEWEST_BLOCK).block;
        const latestBaseFeePerGas = parseInt(block?.baseFeePerGas ?? '0');
        const maxPriorityFeePerGas = ethereumQueries.queryEthereumMaxPriorityFee.maxPriorityFeePerGas;
        if (block != null && latestBaseFeePerGas !== 0 && maxPriorityFeePerGas != null) {
          const baseFeePerGasDec = new Dec(latestBaseFeePerGas).mul(
            ETH_FEE_SETTINGS_BY_FEE_TYPE[feeType].baseFeePercentageMultiplier
          );
          const maxPriorityFeePerGasDec = new Dec(BigInt(maxPriorityFeePerGas));
  ...
  protected populateGasPriceStep(feeCurrency: FeeCurrency, feeType: FeeType): Dec {
  ...
  get uiProperties(): UIProperties {
    ...
    if (
      ...
      const etcQueries = this.queriesStore.get(this.chainId).owalletETC;
      if (etcQueries) {
        if (etcQueries.queryTerraClassicTaxRate.error || etcQueries.queryTerraClassicTaxRate.isFetching) {
          return {
            error: (() => {
            ...
            loadingState: etcQueries.queryTerraClassicTaxRate.isFetching ? 'loading-block' : undefined
          };
        }

        if (etcQueries.queryTerraClassicTaxCaps.error || etcQueries.queryTerraClassicTaxCaps.isFetching) {
          return {
            error: (() => {
            ...
            loadingState: etcQueries.queryTerraClassicTaxCaps.isFetching ? 'loading-block' : undefined
          };
        }
      }
    }
    ...
    if (this.canOsmosisTxFeesAndReady()) {
      ...
      if (queryOsmosis && this.getFeePrimitive().length > 0) {
        ...
        if (
          ...
          const spotPrice = queryOsmosis.queryTxFeesSpotPriceByDenom.getQueryDenom(feeCurrency.coinMinimalDenom);
    ...
    if (this.canEIP1559TxFeesAndReady()) {
      const ethereumQueries = this.queriesStore.get(this.chainId).ethereum;
      if (ethereumQueries) {
        const feeHistoryQuery = ethereumQueries.queryEthereumFeeHistory.getQueryByFeeHistoryParams(
          ETH_FEE_HISTRORY_BLOCK_COUNT,
          ETH_FEE_HISTORY_NEWEST_BLOCK,
          ETH_FEE_HISTORY_REWARD_PERCENTILES
        );
        ...
        if (!feeHistoryQuery.response) {
          const blockQuery =
            ethereumQueries.queryEthereumBlock.getQueryByBlockNumberOrTag(ETH_FEE_HISTORY_NEWEST_BLOCK);
    ...
    if (this.additionAmountToNeedFee) {
      for (let i = 0; i < needs.length; i++) {
        ...
        for (const amt of amount) {
          if (need.currency.coinMinimalDenom === amt.currency.coinMinimalDenom) {
            needs[i] = {
              ...need,
              amount: new Int(need.amount).add(new Int(amt.toCoin().amount)).toString()
            };
          }
        }
      }
    ...
    for (let i = 0; i < needs.length; i++) {
      ...
      const bal = this.queriesStore
        ...
        .balances.find(bal => bal.currency.coinMinimalDenom === need.currency.coinMinimalDenom);

      if (!bal) {
        return {
          warning: new Error(`Can't parse the balance for ${need.currency.coinMinimalDenom}`)
        };
      }
