import { IGasConfig, IGasEthereumConfig } from './types';
import { TxChainSetter } from './chain';
import { ChainGetter } from '@owallet/stores';
import { action, makeObservable, observable } from 'mobx';
import { useState } from 'react';

export class GasEthereumConfig extends TxChainSetter implements IGasEthereumConfig {
  /*
  ...
  constructor(
    ...
    this._gasRaw = initialGas?.toString() || undefined;
...
export const useGasEthereumConfig = (
  chainGetter: ChainGetter,
  chainId: string,
  initialGas?: number
) => {
  const [txConfig] = useState(
    () => new GasEthereumConfig(chainGetter, chainId, initialGas)
  );
