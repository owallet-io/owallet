...
export const signerFromPrivateKey = (privateKey: Uint8Array) => {
  return oasis.signature.NaclSigner.fromSecret(
    privateKey,
...
export const signerFromEthPrivateKey = (ethPrivateKey: Uint8Array) => {
  return oasisRT.signatureSecp256k1.EllipticSigner.fromPrivate(
    ethPrivateKey,
    "this key is not important"
  );
};
...
export class OasisTransaction {
  ...
  public static async signUsingLedger<T>(
    ...
    tw.signedTransaction.signature.signature =
      tw.signedTransaction.signature.signature.slice(0, 64);
  }
  ...
  public static async sign<T>(
    ...
    return tw.sign(
      new oasis.signature.BlindContextSigner(signer),
      chainContext
    );
  }

  public static async signParaTime<T>(
    chainContext: string,
    signer: Signer,
    tw: RTW<T>
  ): Promise<void> {
      ...
      switch (grpcError) {
        ...
        case "consensus: duplicate transaction":
          throw new WalletError(
            WalletErrors.DuplicateTransaction,
  ...
  protected static async getNonce(
    nic: OasisClient,
    signer: Signer
  ): Promise<bigint> {
