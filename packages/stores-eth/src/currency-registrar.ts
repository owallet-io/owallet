...
function isValidCacheWithData(cache: CurrencyCache): cache is {
  notFound: undefined;
  symbol: string;
  decimals: number;
  coingeckoId?: string;
  ...
  return cache.notFound === undefined;
}
...
export class ERC20CurrencyRegistrar {
  ...
  protected getERC20Metadata(
    ...
    if (cached) {
      ...
      if (!staled && isValidCacheWithData(cached)) {
        return {
          res: {
    ...
    if (queries.ethereum) {
      ...
      if (contractInfo) {
        ...
        if (cached && isValidCacheWithData(cached)) {
          cachedRes = {
            coinDenom: cached.symbol,
            decimals: cached.decimals,
            coingeckoId: cached.coingeckoId,
        ...
        if (contractInfo.symbol != null && contractInfo.decimals != null) {
          ...
          if (notFound) {
            const skipTokenInfoQuery =
              queries.owalletETC.querySkipTokenInfo.getQueryCoinMinimalDenom(
