...
import { KeyRingOasisBaseService, KeyRingOasisPrivateKeyService, KeyRingOasisMnemonicService } from './keyring-oasis';
import { KeyRingTronBaseService, KeyRingTronPrivateKeyService, KeyRingTronMnemonicService } from './keyring-tron';
import { KeyRingBtcBaseService, KeyRingBtcPrivateKeyService, KeyRingBtcMnemonicService } from './keyring-bitcoin';
...
export function init(
  ...
  const sidePanelService = new SidePanel.SidePanelService(storeCreator('side-panel'), analyticsService);
  ...
  const tokenCW20Service = new TokenCW20.TokenCW20Service(storeCreator('tokens'), chainsService, interactionService);
  ...
  const backgroundTxService = new BackgroundTx.BackgroundTxService(chainsService, notification);

  const backgroundTxEthereumService = new BackgroundTxEthereum.BackgroundTxEthereumService(chainsService, notification);
  const backgroundTxOasisService = new BackgroundTxOasis.BackgroundTxOasisService(chainsService, notification);
  const backgroundTxTronService = new BackgroundTxTron.BackgroundTxTronService(chainsService, notification);
  ...
  const chainsUIService = new ChainsUI.ChainsUIService(storeCreator('chains-ui'), chainsService, vaultService);
  const keyringBaseMnemonic = new KeyRingMnemonic.KeyRingMnemonicService(vaultService);
  const keyringBasePrivateKey = new KeyRingPrivateKey.KeyRingPrivateKeyService(vaultService);
  ...
  const keyRingV2Service = new KeyRingV2.KeyRingService(
    storeCreator('keyring-v2'),
    {
      ...
      getDisabledChainIdentifiers: keyRingMigrations.getDisabledChainIdentifiers,
      chainsUIService
    },
    chainsService,
    chainsUIService,
    ...
    vaultService,
    analyticsService,
    [keyringBaseMnemonic, keyringBasePrivateKey, keyringBaseLedger, keyringBaseKeystone]
  );
  ...
  const permissionInteractiveService = new PermissionInteractive.PermissionInteractiveService(
  ...
  const recentSendHistoryService = new RecentSendHistory.RecentSendHistoryService(
  ...
  BackgroundTxEthereum.init(router, backgroundTxEthereumService, permissionInteractiveService);
  BackgroundTxOasis.init(router, backgroundTxOasisService, permissionInteractiveService);
  BackgroundTxTron.init(router, backgroundTxTronService, permissionInteractiveService);
  ...
  KeyRingCosmos.init(router, keyRingCosmosService, permissionInteractiveService);
  ...
  KeyRingBitcoin.init(router, keyRingBitcoinService, permissionInteractiveService);
  KeyRingEthereum.init(router, keyRingEthereumService, permissionInteractiveService);
  ...
  TokenCW20.init(router, tokenCW20Service, permissionInteractiveService, keyRingCosmosService);
