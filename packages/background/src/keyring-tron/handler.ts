  ...
  return (env: Env, msg: Message<unknown>) => {
    switch (msg.constructor) {
      ...
      case RequestTriggerSmartContractMsg:
        return handleTriggerSmartContractMsg(
          service,
          permissionInteractionService
        )(env, msg as RequestTriggerSmartContractMsg);
      ...
      case RequestGetTronAddressMsg:
        return handleRequestGetTronAddressMsg(service)(
          env,
          msg as RequestGetTronAddressMsg
        );
      default:
...
const handleTriggerSmartContractMsg: (
  service: KeyRingTronService,
  permissionInteractionService: PermissionInteractiveService
) => InternalHandler<RequestTriggerSmartContractMsg> = (
  service,
  permissionInteractionService
) => {
  return async (env, msg) => {
    ...
    return await service.requestTriggerSmartContract(
      env,
      msg.chainId,
      JSON.parse(msg.data)
    );
