...
describe("keyring", () => {
  ...
  describe("loadKey", () => {
    describe("test for case this.keyStore.type === ledger", () => {
      it("test case for ledgerPublicKey is not null", async () => {
        ...
        expect(Buffer.from(rs.address).toString("hex")).toBe(
          "2a77016e89454aa2b15ae84757e32b75549af840"
        );
    ...
    describe("test case for this.keyStore.type !== ledger", () => {
      it("test for case network type is cosmos", async () => {
        const oraichain = commonOwallet.EmbedChainInfos.filter(
          (item, index) => item.chainId === mockChainId
        )[0];
        Object.defineProperty(keyRing, "chainsService", {
          value: {
            getChainInfo: () => {
              return {
                ...oraichain,
  ...
  describe("getKeyFromCoinType", () => {
    it("getKeyFrom by coin type", async () => {
      Object.defineProperty(keyRing, "status", {
        value: KeyRingStatus.UNLOCKED,
        writable: true,
      });
      keyRing["keyStore"] = mockMultiKeyStore[1];
      keyRing["mnemonic"] = mockKeyCosmos.mnemonic;
      const rs = await keyRing.getKeyFromCoinType(mockCoinType);
      expect(Buffer.from(rs.address).toString("hex")).toBe(
        "cf159c10596b2cc8270da31375d5d741a3c4a949"
      );
      expect(Buffer.from(rs.pubKey).toString("hex")).toBe(
  ...
  describe("getKey", () => {
    it("return data from getKey method", async () => {
      Object.defineProperty(keyRing, "status", {
        value: KeyRingStatus.UNLOCKED,
        writable: true,
      });
      ...
      const rs = await keyRing.getKey(mockChainId, mockCoinType);
  ...
  describe("signEvm", () => {
    ...
    it("sign Tron", () => {
      const mockPrivBytes = new Uint8Array([
        102, 211, 219, 188, 242, 151, 77, 32, 6, 123, 231, 231, 198, 171, 27,
        148, 18, 123, 5, 164, 128, 2, 123, 110, 35, 223, 224, 102, 141, 222,
        136, 126,
      ]);
      ...
      const mockMsg =
        "8f87082466466dbbb310a62c5cb4c66df42b5eb142aaeb9b8ab90add6b617dcb";
  ...
  describe("signEvm", () => {
    ...
    const mockPrivBytes = new Uint8Array([
      102, 211, 219, 188, 242, 151, 77, 32, 6, 123, 231, 231, 198, 171, 27, 148,
      18, 123, 5, 164, 128, 2, 123, 110, 35, 223, 224, 102, 141, 222, 136, 126,
    ]);
    const mockPrivKey: any = {
      toBytes: jest.fn().mockReturnValue(mockPrivBytes),
    };
    const arrayEvm = commonOwallet.EmbedChainInfos.filter(
      (item, index) =>
        item.networkType === "evm" && item.chainId !== "native-0x5afe"
    );
    const arrayMapped = [...arrayEvm].map((item) => {
      ...
      } else if (item.chainId === "0x5afe") {
        (item as any).expected = 23294;
      } else if (item.chainId === "0xa516") {
        (item as any).expected = 42262;
      }
      return item;
    });
    ...
    it("getHexAddressEvm", () => {
      ...
      expect(rs).toEqual([
        "0xad90317473bbc13ba0c9e81d21131fce289078fe",
        "latest",
      ]);
    });
    ...
    const arrayRawTxEvm = [...arrayEvm].map((ite) => {
      ...
      } else if (ite.chainId === "0x5afe") {
        (ite as any).expectedRaw =
          "0xf86d308502540be40083030d4094f2846a1e4dafaea38c1660a618277d67605bd2b58609184e72a0008082b61fa0d95e435eecde037b89fef2d609cf82bd63756406d3edd5da27b32a00e2803258a0068c8528026a39702e7d485fba8c163f55c595202d598a52d6693b12857702c0";
      } else if (ite.chainId === "0xa516") {
        (ite as any).expectedRaw =
          "0xf86e308502540be40083030d4094f2846a1e4dafaea38c1660a618277d67605bd2b58609184e72a0008083014a50a03074b23394502336d073b9db64b935fd9420a36e937c37c6822c383636f7c9a6a0108219b20eac754d8e3bf815a8d19603d0580f66e35808567c428f95aecdde0b";
      }
      return ite;
    });

    it.each(arrayRawTxEvm)("getRawTxEvm for chain: $chainName", (item) => {
      const rs = KeyringHelper.getRawTxEvm(
        mockPrivKey,
        item.chainId,
        "0x30",
        mockMsg
