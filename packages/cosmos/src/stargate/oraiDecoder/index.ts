import { cosmos } from "../oraiProto";
import SignDoc = cosmos.tx.v1beta1.SignDoc;
import { defaultProtoCodec, ProtoCodec } from "../oraiCodec";
...
export class ProtoSignDocDecoder {
  ...
  get txBody(): cosmos.tx.v1beta1.TxBody {
    if (!this._txBody) {
      this._txBody = cosmos.tx.v1beta1.TxBody.decode(this.signDoc.body_bytes);
    }
  ...
  get chainId(): string {
    return this.signDoc.chain_id;
  }

  get accountNumber(): string {
    return this.signDoc.account_number.toString();
  }
