import { encodeSecp256k1Pubkey, encodeSecp256k1Signature, serializeSignDoc, sortedJsonByKeyStringify } from './encode';
...
describe('encode', () => {
  ...
  it('encodeSecp256k1Pubkey', () => {
    const mockPubKey = new Uint8Array([
      2, 59, 29, 39, 166, 47, 187, 235, 126, 180, 207, 194, 215, 129, 131, 199, 191, 29, 95, 73, 53, 59, 110, 89, 145, 226, 147, 129, 32, 119, 195, 210, 149
    ]);
    const output = { type: 'tendermint/PubKeySecp256k1', value: 'AjsdJ6Yvu+t+tM/C14GDx78dX0k1O25ZkeKTgSB3w9KV' };
  ...
  it('encodeSecp256k1Signature', () => {
    ...
    const mockSignature = new Uint8Array([
      15, 87, 121, 209, 52, 40, 88, 235, 85, 160, 107, 2, 2, 125, 251, 16, 215, 51, 94, 25, 159, 80, 47, 126, 14, 249, 150, 211, 80, 18, 85, 130, 173, 122, 34,
      222, 222, 244, 195, 190, 60, 254, 195, 205, 159, 177, 12, 88, 91, 126, 133, 18, 91, 205, 182, 160, 183, 243, 240, 212, 84, 175, 195, 7
    ]);
    const output = {
      pub_key: { type: 'tendermint/PubKeySecp256k1', value: 'AjsdJ6Yvu+t+tM/C14GDx78dX0k1O25ZkeKTgSB3w9KV' },
      signature: 'D1d50TQoWOtVoGsCAn37ENczXhmfUC9+DvmW01ASVYKteiLe3vTDvjz+w82fsQxYW36FElvNtqC38/DUVK/DBw=='
    };
